...
namespace Dijkstra
{
    ...
    std::vector<int> Para_Di(int start, int nodes, Thread_Pool &pool, std::atomic<int> &_threads)
    {
        ...
        std::vector<std::atomic<bool>> in_queue(nodes);
        ...
        while (!relaxed_nodes.empty())
        {
            ...
            for (int st = 0; st < sz; st += block_size)
            {
                ...
                for(int i = st; i < end; i++){
                    ...
                    in_queue[u].store(false, std::memory_order_relaxed);
                    ...
                    for(auto &[v, w] : Adj[u]){
                        ...
                        while(old_dist > new_dist){
                            if (__atomic_compare_exchange_n(&dist[v], &old_dist, new_dist, false, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
                            {
                                ...
                                if (!in_queue[v].exchange(true, std::memory_order_relaxed)) {
